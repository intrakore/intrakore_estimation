...
class UniversalBOQParser:
    ...
    def _parse_quantity(self, qty: Any) -> float:
        ...
        # Handle float with precision issues (like 134.10000000000001)
        ...
        # Remove trailing .0 or .100 from string representation
        if '.' in qty_str:
    ...
    def _get_cell_value(self, row: pd.Series, idx: Optional[int]) -> str:
        ...
        if pd.isna(val):
            return ""
        # If it's a float with precision issues, fix it
        if isinstance(val, float):
            # Round to 2 decimal places and convert to string without trailing zeros
            rounded = round(val, 2)
            if rounded == int(rounded):
